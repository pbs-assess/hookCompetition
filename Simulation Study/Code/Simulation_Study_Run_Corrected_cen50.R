...
ggplot(aes(x=Station, y=Mean, ymin=LCL, ymax=UCL, colour=model, group=model, shape=model)) +
  ...
  ggtitle('Bias in Relative Abundance Indices vs Method',
          subtitle = 'Rows are trends in relative abundance of target species and the average degree of hook saturation,\nColumns describe both the arrival time distributions and the abilities to locate baited hooks after 50% of baits removed') +
  ...
  scale_color_viridis_d(labels=c('Naive','Adjusted','Censored 85%','Censored 95%')) +
  scale_shape_manual(labels=c('Naive','Adjusted','Censored 85%','Censored 95%'),
...
multiplot(
  Results %>%
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          panel.background = element_blank(), axis.line = element_blank(),
          legend.position = 'right') +
    scale_color_viridis_d(labels=c('Adjusted','Censored 85%','Censored 95%')) +
    scale_shape_manual(labels=c('Adjusted','Censored 85%','Censored 95%'),
